...
const db = spicedPg(
  process.env.DATABASE_URL ||
    'postgres:postgres:postgres@localhost:5432/sodolinskyi'
);
...
module.exports.deleteYinCustomer = (id) => {
  return db
    .query(`DELETE FROM yinusers WHERE yinusers.id = $1`, [id])
    .catch((error) => {
      console.log(error.message);
    });
};
...
module.exports.deleteUser = (id) => {
  return db
    .query(`DELETE FROM users WHERE users.id = $1`, [id])
    .catch((error) => {
      console.log(error.message);
    });
};
...
module.exports.getNotes = () => {
  return db.query(`SELECT * FROM notes`, []).catch((error) => {
    console.log(error);
  });
};
